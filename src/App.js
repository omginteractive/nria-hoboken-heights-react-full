...
import { setOperatingSys, setBrowser, changeIsMobileDevice, setHeader } from "./redux/actions/appActions";
  ...
  constructor(props) {
    ...
    componentDidMount() {
        ...
        flypilotFetchWPRestAPI(endpoint, apartmentsEndpoint).then((result)=> {
            ...
            this.props.setHeader(result['headerData'])
            this.handleResize()
        })
    	...
    	window.addEventListener('keydown', (event) => {
			if (!event.target.classList.contains('input')) {
				...
				if (keyboardCommand === "ArrowUp") result = this.prevSlide()
				else if (keyboardCommand === "ArrowDown") result = this.nextSlide()
				else if (keyboardCommand === "ArrowLeft") result = this.prevSlide()
				else if (keyboardCommand === "ArrowRight") result = this.nextSlide()
                ...
                if(result === 'needScroll') {
                    ...
                    if(keyboardCommand === "ArrowDown" || keyboardCommand === "ArrowRight") {
                        ...
                        this.triggerScrollUpElement(elementToScroll)
                    }
                }
			}
        });
...
const mapStateToProps = state => {
  ...
  export default connect(
    mapStateToProps,
    { setHeader, setOperatingSys, setBrowser, findDeviceSlideIdx, changeSlideIdx, updateMenuData, updateSlideData, updateSlideTransitioningState, updateSlideTouchState, updateDesktopKeys, updateMobileKeys, changeIsMobileDevice, updateSlidesViewed }
  )(App);